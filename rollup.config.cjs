...
function createOutput(preserveModules = true, preserveModulesRoot = "src") {
  /**
  ...
  if (!preserveModules) {
    output.forEach((o) => {
      // Delete the preserveModulesRoot from the output
      o.preserveModulesRoot = undefined;
    });
  }
  return output;
}
...
for (const entry of entries) {
  const config = {
    input: entry,
    output: createOutput(true, "src"),
    plugins,
    external: externalFilter
  };
  // Check if extension is .ts
